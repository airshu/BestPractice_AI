...
class CronScheduler:
    ...
    def _check_tasks(self, now: datetime):
        ...
        if expired or fired_oneshots:
            ...
            for tid in expired:
                print(
                    f"[Cron] Auto-expired: {tid} (older than {AUTO_EXPIRY_DAYS} days)"
                )
...
if __name__ == "__main__":
    ...
    while True:
        ...
        if query.strip() == "/test":
            ...
            scheduler.queue.put(
                "[Scheduled task test-0000]: This is a test notification."
            )
            print(
                "[Test notification enqueued. It will be injected on your next message.]"
            )
            continue
