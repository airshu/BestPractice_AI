    ...
    def normalize(self, tool_name: str, tool_input: dict) -> dict:
        ...
        if actual_tool == "read_file" or lowered.startswith(self.READ_PREFIXES):
            ...
            risk = (
                ...
                if any(
                    token in command
                    for token in ("rm -rf", "sudo", "shutdown", "reboot")
                )
                else "write"
            )
...
permission_gate = CapabilityPermissionGate(mode="auto")
    ...
    def connect(self):
        ...
        try:
            self.process = subprocess.Popen(
                ...
                stderr=sys.stderr,  # 直接输出到主进程 stderr
                env=self.env,
                text=True,
            )
...
class MCPToolRouter:
    ...
    def call(self, tool_name: str, arguments: dict) -> str:
        ...
        Format: mcp__{plugin}__{server}__{tool} or mcp__{server}__{tool}
        We need to match registered server names which may contain underscores.
        """
        if not tool_name.startswith("mcp__"):
            return f"Error: Not an MCP tool: {tool_name}"
        
        prefix, rest = tool_name.split("__", 1)
        ...
        if not client:
            # Try matching server name with plugin prefix stripped
            for registered_name, c in self.clients.items():
                if registered_name.endswith(f"__{server_name}") or registered_name == server_name:
                    client = c
                    break
...
def normalize_tool_result(
    tool_name: str, output: str, intent: dict | None = None
) -> str:
