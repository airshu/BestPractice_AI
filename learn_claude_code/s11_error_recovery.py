...
MAX_OUTPUT_TOKENS = 2000  # 测试用小值，生产环境改为 8000
        ...
        for attempt in range(MAX_RECOVERY_ATTEMPTS + 1):
            try:
                ...
                if "overlong_prompt" in error_body or (
                    "prompt" in error_body and "long" in error_body
                ):
                    print(
                        f"[Recovery] Prompt too long. Compacting... (attempt {attempt + 1})"
                    )
                ...
                print(
                    f"[Error] API call failed after {MAX_RECOVERY_ATTEMPTS} retries: {e}"
                )
                return
                ...
                if attempt < MAX_RECOVERY_ATTEMPTS:
                    delay = backoff_delay(attempt)
                    print(
                        f"[Recovery] Connection error: {e}. "
                        f"Retrying in {delay:.1f}s (attempt {attempt + 1}/{MAX_RECOVERY_ATTEMPTS})"
                    )
                    time.sleep(delay)
                    continue
                print(
                    f"[Error] Connection failed after {MAX_RECOVERY_ATTEMPTS} retries: {e}"
                )
                return
        if response is None:
