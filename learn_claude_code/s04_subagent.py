...
def run_subagent(prompt: str) -> str:
    print("\n\033[32m========== SUBAGENT STARTED ==========\033[0m")
    print(f"\033[32m[SUBAGENT] Task: {prompt[:100]}...\033[0m")
    sub_messages = [{"role": "user", "content": prompt}]  # fresh context
    subagent_turn = 0
    for _ in range(30):  # safety limit
        subagent_turn += 1
        print(f"\033[32m[SUBAGENT] Turn {subagent_turn}...\033[0m")
        ...
        pretty_print("sub_messages:", sub_messages)
    ...
    print(f"\033[32m[SUBAGENT] Finished, returning summary ({len(response.content)} content blocks)\033[0m")
    print("\033[32m========== SUBAGENT ENDED ==========\033[0m\n")
    return (
    ...
    while True:
        ...
        for block in response.content:
            if block.type == "tool_use":
                ...
                # 日志：task 工具被调用
                if block.name == "task":
                    print(f"\033[33m[PARENT] task tool called with prompt: {block.input.get('prompt', '')[:50]}...\033[0m")
