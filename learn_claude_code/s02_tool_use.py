...
def build_client() -> Anthropic:
    ...
    if not auth_token:
        raise RuntimeError("Missing auth token. Please set ANTHROPIC_AUTH_TOKEN")

    # Anthropic SDK 会自动拼接 /v1，避免传入后形成 /v1/v1。
...
WORKDIR = Path.cwd()
client = build_client()
MODEL = os.getenv("ANTHROPIC_MODEL_ID") or os.getenv("MODEL_ID") or "claude-sonnet-4-6"
...
def agent_loop(messages: list):
    while True:
        try:
            ...
            text = str(exc)
            if "Error code: 400" in text and "正确的 Claude Code 客户端" in text:
                text = (
                    "Model call failed: gateway rejected Anthropic Messages request.\n"
                    "Hint: this base_url likely requires provider-specific client/protocol,\n"
                    "or use official Anthropic endpoint: https://api.anthropic.com"
                )
            messages.append(
